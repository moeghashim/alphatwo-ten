...
Tool-CLI spec: [`CONTRACT.md`](CONTRACT.md) — the versioned contract every tool conforms to (the detail behind design §9–11). Building tools: the [`build-cli-tool`](https://github.com/moeghashim/build-cli-tool) repo — a clonable template + agent skill (the build-a-tool + build-the-kit briefs). Brand mark: [`branding/tenwhy.svg`](branding/tenwhy.svg). Legacy URL redirects: [`_redirects`](_redirects).
